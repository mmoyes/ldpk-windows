...
//! @file tde4_ldp_radial_fisheye_equisolid_deg_8.h
//! @brief Plugin class for radial distortion

//! @brief Plugin class for radial distortion.
//! Does not compensate for decentering.
template <class VEC2,class MAT2>
class tde4_ldp_radial_fisheye_equisolid_deg_8:public tde4_ldp_radial_fisheye_base_deg_8<VEC2,MAT2>
	{
private:
...
// Now remap to distance in equisolid lens
		r_ed_dn = 2.0 * _fl_dn * ::sin(theta * (1.0/2.0));
		return true;
		}
	bool getModelName(char *name)
		{
#ifdef LDPK_COMPILE_AS_PLUGIN_SDV
		strcpy(name,"3DE4 Radial - Fisheye, Equisolid, Degree 8 [Plugin]");
#else
		strcpy(name,"3DE4 Radial - Fisheye, Equisolid, Degree 8");
#endif
		return true;
		}
	};
