...
//! @file tde4_ldp_radial_fisheye_equidistant_longlat_y_deg_8.h
//! @brief Plugin class for radial distortion

//! @brief Plugin class for radial distortion.
//! Does not compensate for decentering.
template <class VEC2,class MAT2>
class tde4_ldp_radial_fisheye_equidistant_longlat_y_deg_8:public ldpk::ldp_builtin<VEC2,MAT2>
	{
private:
	typedef VEC2 vec2;
	typedef MAT2 mat2;
...
// Coordinates in longlat image.
		double phi = ::atan2(p_ed_dist_dn[1],p_ed_dist_dn[0]);
		double the = the_undist;
// We need phi in [0,2pi[.
		if(phi < 0)
			{ phi += 2.0 * M_PI; }
	...
	bool distort(double x0,double y0,double &x1,double &y1)
		...
		double r_ed_dist_dn = _fl_dn * _distortion.map_inverse(the);
		vec2 p_ed_dn = r_ed_dist_dn * vec2(::cos(phi),::sin(phi));
...
// Mutex initialized and destroyed in baseclass.
	tde4_ldp_radial_fisheye_equidistant_longlat_y_deg_8():_r_clip_factor(50.0)
		{ }
	~tde4_ldp_radial_fisheye_equidistant_longlat_y_deg_8()
		{ }
	...
	bool getModelName(char *name)
		...
		strcpy(name,"3DE4 Radial - Fisheye, Equidistant, Longlat, Degree 8 [Plugin]");
#else
		strcpy(name,"3DE4 Radial - Fisheye, Equidistant, Longlat, Degree 8");
#endif
		return true;
		}
...
template <class VEC2,class MAT2>
const char* tde4_ldp_radial_fisheye_equidistant_longlat_y_deg_8<VEC2,MAT2>::_para[4] = {
	"Degree 2",
	"Degree 4",
	"Degree 6",
	"Degree 8"
