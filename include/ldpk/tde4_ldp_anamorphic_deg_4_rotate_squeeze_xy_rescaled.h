...
#ifndef tde4_ldp_anamorphic_deg_4_rotate_squeeze_xy_rescaled_sdv
#define tde4_ldp_anamorphic_deg_4_rotate_squeeze_xy_rescaled_sdv
...
//! @file tde4_ldp_anamorphic_deg_4_rotate_squeeze_xy_rescaled.h
//! Degree-4 anamorphic model with anamorphic lens rotation
//! For anamorphic footage rescaled to pixel aspect 1.

template <class VEC2,class MAT2>
class tde4_ldp_anamorphic_deg_4_rotate_squeeze_xy_rescaled:public ldpk::ldp_builtin<VEC2,MAT2>
	{
private:
...
// mit einer Rotation und einem Squeeze. Das ist der anamorphe Anteil
	...
	ldpk::squeeze_x_extender<vec2_type,mat2_type> _pa,_rescale;

//! Concatenating extenders for better performance
	ldpk::linear_extender<vec2_type,mat2_type> _rot_sqx_sqy_rescale_pa;
	ldpk::linear_extender<vec2_type,mat2_type> _pa_rescale_rot;

	static const char* _para[14];
...
// therefore we have to prepare the concatenated extenders here.
		...
		if(_squeeze_x.get_sq() == 0)
			{ std::cerr << "tde4_ldp_anamorphic_deg_4_rotate_squeeze_xy_rescaled::initializeParameters, error: Squeeze-X is 0." << std::endl; }
		if(_squeeze_y.get_sq() == 0)
			{ std::cerr << "tde4_ldp_anamorphic_deg_4_rotate_squeeze_xy_rescaled::initializeParameters, error: Squeeze-Y is 0." << std::endl; }
...
// Mutex initialized and destroyed in baseclass.
	tde4_ldp_anamorphic_deg_4_rotate_squeeze_xy_rescaled()
		{ }
	~tde4_ldp_anamorphic_deg_4_rotate_squeeze_xy_rescaled()
		{ }
	bool getModelName(char *name)
		...
		strcpy(name,"3DE4 Anamorphic - Rescaled, Degree 4 [Plugin]");
#else
		strcpy(name,"3DE4 Anamorphic - Rescaled, Degree 4");
#endif
		return true;
		}
...
//! Tested against difference quotients.
	bool getJacobianMatrix(double x0,double y0,double& m00,double& m01,double& m10,double& m11)
		{
		typedef base_type bt;
		mat2_type m = _rot_sqx_sqy_rescale_pa.get_mat()
					* _anamorphic.jacobi(
						_pa_rescale_rot.eval_inv(
							bt::map_unit_to_dn(vec2_type(x0,y0))))
...
template <class VEC2,class MAT2>
const char* tde4_ldp_anamorphic_deg_4_rotate_squeeze_xy_rescaled<VEC2,MAT2>::_para[14] = {
