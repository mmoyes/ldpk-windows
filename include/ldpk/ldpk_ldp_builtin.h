...
namespace ldpk
	{
//! @brief An enum-type for built-in parameters. Also used to address elements in parameter name array.
...
//! The lookup table. At some point befor LDPK 2.0 we had to specify
//! more details: The LUT contains initial values for the gnomonic
//! distort-method. The idea is the following: For non-gnomonic cameras
//! like fisheye we have to take separate clearly the model function and
//! the projection. All model functions are implicitely defined for
//! gnomonic projection, and we realize e.g. equisolid by an appropriate
//! re-mapping. When we compute the inverse of undistort(), we invert
//! both the model function (which is gnomonic) and the projection
//! and concatenate them (see tde4_ldp_radial_deg_8.h). Hence, we
//! need initial values for the (gnomonic) distort-method.
...
//! when some parameter was changed.
		void no_longer_uptodate_lut()
			{ _uptodate_lut = false; }
//! @brief If a derived class computes the LUT, it must have write-access to this flag.
...
//! Our base class presents the model functions along with the projection.
//! Internally, we have to carefully keep apart the model function and the projection. 
//! For most distortion models, this is no problem, yet for fisheye models it is,
//! so all non-gnomonic distortion classes will have to overwrite these.
//@{
...
// Call the inverse model function with initial values.
			distort_gnomonic(p[0],p[1],qs[0],qs[1],q[0],q[1]);
			_lut.set_q_current_fov(q);
			}
		set_uptodate_lut(true);
		}
	}
