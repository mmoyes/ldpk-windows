...
//! @brief returns a name for the model as to show up in the GUI (maximum length of "name": 100 bytes)...
	virtual bool getModelName(char *model) = 0;

//! @brief returns the number of plugin parameters...
	virtual bool getNumParameters(int &n) = 0;

//! @brief returns "identifier" name of parameter "i" (maximum length of "identifier": 100 bytes)...
...
//! @brief returns default value for given parameter (maximum length of "char *v": 1000 bytes)......
...
//! @brief returns range for adjustable double parameters...
...
//! @brief prepare the current set of parameters...
	virtual bool initializeParameters() = 0;

//! @brief warp/unwarp 2D points...
...
//! @brief potentially more efficient function which uses initial values...
	virtual bool distort(double x0, double y0, double x1_start , double y1_start, double &x1, double &y1)
		{ return distort(x0,y0,x1,y1); }

//! @brief parameter derivatives to be used for distortion grid controls' matrix calculation routine...
	virtual bool providesParameterDerivatives()
		{ return false; }
//! @brief dx_dy is an array with 2 * num_parameters elements (x- and y- component for each of n derivatives)
...
//! The model function is evaluated at four points forming a little cross. If any of these evaluations
//! fail (e.g. domain error), the method returns false, else true.
...
//! The model function is evaluated at four points forming a little rectangle. If any of these evaluations
//! fail (e.g. domain error), the method returns false, else true.
	...
	virtual bool getTwistVector(double x0, double y0, double &t0, double &t1)
		{
		return getTwistVectorDQ(x0,y0,t0,t1);
		}
//! @brief Iterate around the specified box, undistort the points and compute the bounding box
...
// @brief Avoid double evaluation at the corners.
...
//! @brief Iterate around the specified box, distort the points and compute the bounding box
