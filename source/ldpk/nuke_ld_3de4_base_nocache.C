...
// No extra margin for the nocache-version.
	return DD::Image::Box(int(floor(xa_px)),int(floor(ya_px)),int(ceil(xb_px)),int(ceil(yb_px)));
	}
...
// Compute resulting row.
		for(int i = 0;i < pos.size();++i)
			...
			if(pos[i].status())
				{
// Transfer the stored result. We set pixels x to r in outrow.
				in0.sample(pos[i].x(),pos[i].y(),1.0,1.0,&_filter,out);
				foreach(z,channels)
