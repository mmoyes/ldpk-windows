...
#include <ldpk/tde4_ldp_radial_homomorphic_decentered_deg_2.experimental.h>
#include <ldpk/tde4_ldp_radial_fisheye_equidistant_deg_8.experimental.h>
#include <ldpk/tde4_ldp_radial_fisheye_equisolid_deg_8.experimental.h>
#include <ldpk/tde4_ldp_radial_fisheye_orthographic_deg_8.experimental.h>
#include <ldpk/tde4_ldp_radial_fisheye_stereographic_deg_8.experimental.h>
...
static int anamorphic_deg_4_rotate_squeeze_xy_rescaled(tde4_ldp_py_wrapper *self, PyObject *args, PyObject *kwds)
	{
	self->_ldm = new tde4_ldp_anamorphic_deg_4_rotate_squeeze_xy_rescaled<ldpk::vec2d,ldpk::mat2d>();
	return 0;
	}
...
* Experimental
***************************************************************/
static int radial_homomorphic_decentered_deg_2(tde4_ldp_py_wrapper *self, PyObject *args, PyObject *kwds)
	{
	self->_ldm = new tde4_ldp_radial_homomorphic_decentered_deg_2<ldpk::vec2d,ldpk::mat2d>();
	return 0;
	}
static int radial_fisheye_equidistant_deg_8(tde4_ldp_py_wrapper *self, PyObject *args, PyObject *kwds)
	{
	self->_ldm = new tde4_ldp_radial_fisheye_equidistant_deg_8<ldpk::vec2d,ldpk::mat2d>();
	return 0;
	}
static int radial_fisheye_equisolid_deg_8(tde4_ldp_py_wrapper *self, PyObject *args, PyObject *kwds)
	{
	self->_ldm = new tde4_ldp_radial_fisheye_equisolid_deg_8<ldpk::vec2d,ldpk::mat2d>();
	return 0;
	}
static int radial_fisheye_orthographic_deg_8(tde4_ldp_py_wrapper *self, PyObject *args, PyObject *kwds)
	{
	self->_ldm = new tde4_ldp_radial_fisheye_orthographic_deg_8<ldpk::vec2d,ldpk::mat2d>();
	return 0;
	}
static int radial_fisheye_stereographic_deg_8(tde4_ldp_py_wrapper *self, PyObject *args, PyObject *kwds)
	{
	self->_ldm = new tde4_ldp_radial_fisheye_stereographic_deg_8<ldpk::vec2d,ldpk::mat2d>();
	return 0;
	}
/*********************************/
...
// Check range of i.
	int num_parameters;
	self->_ldm->getNumParameters(num_parameters);
	if((i_par < 0) || (i_par >= num_parameters))
		{
		PyErr_SetString(PyExc_RuntimeError,"from " __FILE__ ", getParameterName: index out of range.");
		return NULL;
		}
// Get parameter name
	char name_par[256];
...
static PyObject* ldm_undistort(tde4_ldp_py_wrapper* self,PyObject* args)
	...
	if(!self->is_initialized())
		{ return self->not_initialized(); }
	if(!self->_ldm->undistort(x0,y0,x1,y1))
...
static PyObject* ldm_distort(tde4_ldp_py_wrapper* self,PyObject* args)
	...
	if(!self->is_initialized())
		{ return self->not_initialized(); }
	if(!self->_ldm->distort(x0,y0,x1,y1))
		{ Py_INCREF(Py_None);return Py_None; }
	PyObject *t = Py_BuildValue("dd",x1,y1);
	return t;
	}
static PyObject* ldm_getJacobianMatrix(tde4_ldp_py_wrapper* self,PyObject* args)
	...
	if(!self->is_initialized())
		{ return self->not_initialized(); }
	if(!self->_ldm->getJacobianMatrix(x0,y0,m00,m01,m10,m11))
		{ Py_INCREF(Py_None);return Py_None; }
	PyObject *t = Py_BuildValue("dddd",m00,m01,m10,m11);
	return t;
	}
static PyObject* ldm_getJacobianMatrixDQ(tde4_ldp_py_wrapper* self,PyObject* args)
	{
	double x0,y0,m00,m01,m10,m11;
	if(!PyArg_ParseTuple(args,"(dd)",&x0,&y0))
		{ return NULL; }
	if(!self->is_initialized())
		{ return self->not_initialized(); }
	if(!self->_ldm->getJacobianMatrixDQ(x0,y0,m00,m01,m10,m11))
		{ Py_INCREF(Py_None);return Py_None; }
	PyObject *t = Py_BuildValue("dddd",m00,m01,m10,m11);
	return t;
	}
static PyObject* ldm_getTwistVector(tde4_ldp_py_wrapper* self,PyObject* args)
	...
	if(!self->is_initialized())
		{ return self->not_initialized(); }
	if(!self->_ldm->getTwistVector(x0,y0,t0,t1))
		{ Py_INCREF(Py_None);return Py_None; }
	PyObject *t = Py_BuildValue("dd",t0,t1);
	return t;
	}
static PyObject* ldm_getTwistVectorDQ(tde4_ldp_py_wrapper* self,PyObject* args)
	{
	double x0,y0,t0,t1;
	if(!PyArg_ParseTuple(args,"(dd)",&x0,&y0))
		{ return NULL; }
	if(!self->is_initialized())
		{ return self->not_initialized(); }
	if(!self->_ldm->getTwistVectorDQ(x0,y0,t0,t1))
		{ Py_INCREF(Py_None);return Py_None; }
	PyObject *t = Py_BuildValue("dd",t0,t1);
	return t;
	}
...
static PyMethodDef ldm_methods[] = {
	...
	{"getJacobianMatrix", (PyCFunction)ldm_getJacobianMatrix, METH_VARARGS,
		...
		"Invokes getJacobianMatrixDQ(), if the distortion model does not provide an implementation."
		},
	{"getJacobianMatrixDQ", (PyCFunction)ldm_getJacobianMatrixDQ, METH_VARARGS,
		"Signature: getJacobianMatrixDQ((x,y)) -> (double,double,double,double).\n"
		"Jacobian for point (x,y) by difference quotients. Result is (m00,m01,m10,m11) <- this order. "
		"This is the Jacobian for the undistort-method, in unit coordinates [0,1]x[0,1]."
		},
	{"getTwistVector", (PyCFunction)ldm_getTwistVector, METH_VARARGS,
		...
		"Same as getTwistVectorDQ, if the distortion model does not provide an implementation."
		},
	{"getTwistVectorDQ", (PyCFunction)ldm_getTwistVectorDQ, METH_VARARGS,
		"Signature: getTwistVectorDQ((x,y)) -> (double,double).\n"
		"Mixed Derivatives for point (x,y) by difference quotients. Result is (t0,t1). "
		"These are the mixed derivatives for the undistort-method, in unit coordinates [0,1]x[0,1]."
		},
...
static PyTypeObject anamorphic_deg_4_rotate_squeeze_xy_rescaled_Type = {
	PyObject_HEAD_INIT(NULL)
	0,                         /*ob_size*/
	"lens_distortion_plugins.tde4_ldp_py_wrapper",             /*tp_name*/
	sizeof(tde4_ldp_py_wrapper),             /*tp_basicsize*/
	...
	0,                         /* tp_descr_set */
	0,                         /* tp_dictoffset */
	(initproc)anamorphic_deg_4_rotate_squeeze_xy_rescaled,      /* tp_init */
	0,                         /* tp_alloc */
	ldm_new,                 /* tp_new */
	};
...
// Experimental
static PyTypeObject radial_homomorphic_decentered_deg_2_Type = {
	PyObject_HEAD_INIT(NULL)
	0,                         /*ob_size*/
	"lens_distortion_plugins.tde4_ldp_py_wrapper",             /*tp_name*/
	sizeof(tde4_ldp_py_wrapper),             /*tp_basicsize*/
	...
	0,                         /* tp_descr_set */
	0,                         /* tp_dictoffset */
	(initproc)radial_homomorphic_decentered_deg_2,      /* tp_init */
	0,                         /* tp_alloc */
	ldm_new,                 /* tp_new */
	};
static PyTypeObject radial_fisheye_equidistant_deg_8_Type = {
	PyObject_HEAD_INIT(NULL)
	0,                         /*ob_size*/
	"lens_distortion_plugins.tde4_ldp_py_wrapper",             /*tp_name*/
	sizeof(tde4_ldp_py_wrapper),             /*tp_basicsize*/
	...
	0,                         /* tp_descr_set */
	0,                         /* tp_dictoffset */
	(initproc)radial_fisheye_equidistant_deg_8,      /* tp_init */
	0,                         /* tp_alloc */
	ldm_new,                 /* tp_new */
	};
static PyTypeObject radial_fisheye_equisolid_deg_8_Type = {
	PyObject_HEAD_INIT(NULL)
	0,                         /*ob_size*/
	"lens_distortion_plugins.tde4_ldp_py_wrapper",             /*tp_name*/
	sizeof(tde4_ldp_py_wrapper),             /*tp_basicsize*/
	...
	0,                         /* tp_descr_set */
	0,                         /* tp_dictoffset */
	(initproc)radial_fisheye_equisolid_deg_8,      /* tp_init */
	0,                         /* tp_alloc */
	ldm_new,                 /* tp_new */
	};
static PyTypeObject radial_fisheye_orthographic_deg_8_Type = {
	PyObject_HEAD_INIT(NULL)
	0,                         /*ob_size*/
	"lens_distortion_plugins.tde4_ldp_py_wrapper",             /*tp_name*/
	sizeof(tde4_ldp_py_wrapper),             /*tp_basicsize*/
	...
	0,                         /* tp_descr_set */
	0,                         /* tp_dictoffset */
	(initproc)radial_fisheye_orthographic_deg_8,      /* tp_init */
	0,                         /* tp_alloc */
	ldm_new,                 /* tp_new */
	};
static PyTypeObject radial_fisheye_stereographic_deg_8_Type = {
	PyObject_HEAD_INIT(NULL)
	0,                         /*ob_size*/
	"lens_distortion_plugins.tde4_ldp_py_wrapper",             /*tp_name*/
	sizeof(tde4_ldp_py_wrapper),             /*tp_basicsize*/
	...
	0,                         /* tp_descr_set */
	0,                         /* tp_dictoffset */
	(initproc)radial_fisheye_stereographic_deg_8,      /* tp_init */
	0,                         /* tp_alloc */
	ldm_new,                 /* tp_new */
	};
...
PyMODINIT_FUNC initlens_distortion_plugins(void)
	...
	if (PyType_Ready(&anamorphic_deg_4_rotate_squeeze_xy_Type) < 0)
		return;
	if (PyType_Ready(&anamorphic_deg_4_rotate_squeeze_xy_rescaled_Type) < 0)
		return;
	...
	if (PyType_Ready(&all_par_types_Type) < 0)
		return;
// Experimental
	if (PyType_Ready(&radial_homomorphic_decentered_deg_2_Type) < 0)
		return;
	if (PyType_Ready(&radial_fisheye_equidistant_deg_8_Type) < 0)
		return;
	if (PyType_Ready(&radial_fisheye_equisolid_deg_8_Type) < 0)
		return;
	if (PyType_Ready(&radial_fisheye_orthographic_deg_8_Type) < 0)
		return;
	if (PyType_Ready(&radial_fisheye_stereographic_deg_8_Type) < 0)
		return;
	...
	Py_INCREF(&anamorphic_deg_4_rotate_squeeze_xy_rescaled_Type);
	PyModule_AddObject(m,"anamorphic_deg_4_rotate_squeeze_xy_rescaled",	(PyObject *)&anamorphic_deg_4_rotate_squeeze_xy_rescaled_Type);
...
// Experimental
	Py_INCREF(&radial_homomorphic_decentered_deg_2_Type);
	PyModule_AddObject(m,"radial_homomorphic_decentered_deg_2",	(PyObject *)&radial_homomorphic_decentered_deg_2_Type);

	Py_INCREF(&radial_fisheye_equidistant_deg_8_Type);
	PyModule_AddObject(m,"radial_fisheye_equidistant_deg_8",	(PyObject *)&radial_fisheye_equidistant_deg_8_Type);

	Py_INCREF(&radial_fisheye_equisolid_deg_8_Type);
	PyModule_AddObject(m,"radial_fisheye_equisolid_deg_8",	(PyObject *)&radial_fisheye_equisolid_deg_8_Type);

	Py_INCREF(&radial_fisheye_orthographic_deg_8_Type);
	PyModule_AddObject(m,"radial_fisheye_orthographic_deg_8",	(PyObject *)&radial_fisheye_orthographic_deg_8_Type);

	Py_INCREF(&radial_fisheye_stereographic_deg_8_Type);
	PyModule_AddObject(m,"radial_fisheye_stereographic_deg_8",	(PyObject *)&radial_fisheye_stereographic_deg_8_Type);
	}
