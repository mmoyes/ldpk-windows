...
mat2 rot_sqx_sqy_rescale_pa = rotate_mat(radians(lens_rotation)) * squeeze_x_mat(squeeze_x) * squeeze_y_mat(squeeze_y) * squeeze_x_mat(rescale) * squeeze_x_mat(tde4_pixel_aspect);
mat2 pa_rescale_rot = squeeze_x_mat(tde4_pixel_aspect) * squeeze_x_mat(rescale) * rotate_mat(radians(lens_rotation));
mat2 pa_rescale_rot_inv = invert(pa_rescale_rot);

vec2 eval_rot_sqx_sqy_rescale_pa(vec2 p)
	{ return rot_sqx_sqy_rescale_pa * p; }
vec2 eval_pa_rescale_rot_inv(vec2 p)
	{ return pa_rescale_rot_inv * p; }
...
vec2 undistort_diag(vec2 p_diag)
	{
	return eval_rot_sqx_sqy_rescale_pa(anamorphic(eval_pa_rescale_rot_inv(p_diag)));
	}
mat2 jacobian(vec2 p_diag)
	{
	vec2 p = eval_pa_rescale_rot_inv(p_diag);
	float x = p[0];
	float x2 = x * x;
	float x3 = x2 * x;
	float x4 = x2 * x2;
...
// and physics, so that matrix multiplication is (a b)[i][k] = \sum_j a[i][j] b[j][k]
	...
	return rot_sqx_sqy_rescale_pa * m * pa_rescale_rot_inv;
	}
