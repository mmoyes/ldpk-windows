...
def gen_dir_entries_html(directory,format,base,hrefpath,cssprefix,dironly,charset,order,include_files,exclude_files,exclude_dirs,skip,links):
...
# l ist sortiert
	if format == "plain":
		for e in l:
			if os.path.isdir(directory + "/" + e) == True:
				yield " " + e,True,e == l[-1]
				for u,is_child,is_last in gen_dir_entries_html(directory + "/" + e,format,base,hrefpath + "/" + e,cssprefix,dironly,charset,order,include_files,exclude_files,exclude_dirs,skip,links):
					if is_child == True:
						for i in range(skip):
							if charset == "utf-8":
								yield "|",False,e == l[-1]
...
def make_file_list(directory,format,base,cssprefix,dironly,charset,order,include_files,exclude_files,exclude_dirs,skip,links):
	dirbase = os.path.basename(os.path.realpath(directory))
	g = gen_dir_entries_html(directory,format,base,dirbase,cssprefix,dironly,charset,order,include_files,exclude_files,exclude_dirs,skip,links)
	if format == "plain":
		print dirbase
...
parser.add_argument("-format",dest = "format",default = "plain",help = "possible values are: 'html', 'plain'")
...
parser.add_argument("-include_files",dest = "include_files",nargs = "*",help = "files to include (python regex)")
parser.add_argument("-exclude_files",dest = "exclude_files",nargs = "*",help = "files to exclude (python regex)")
parser.add_argument("-exclude_dirs",dest = "exclude",nargs = "*",help = "dirs to exclude (python regex)")
...
if len(sys.argv) == 1:
	...
	dironly = True if args.dironly == "True" else False
	make_file_list(args.path,args.format,args.linkbase,args.cssprefix,dironly,args.charset,args.order,args.include_files,args.exclude_files,args.exclude,int(args.skip),bool(args.links))
